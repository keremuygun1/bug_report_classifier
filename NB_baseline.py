...
for project in projects:
    ...
    out_csv_name = f'./NB_results/{project}_NB.csv'
    ...
    #try 13 different smoothing strengths and let GridSearchCV figure out which one works best.
    #var_smoothing is needed in this case since the variance for the data is near zero, making the probability 
    #calculations rocket to values such as 97 percent.
    #np.logspace(-12, 0, 13) generates 13 numbers evenly spaced on a logarithmic scale, from 10⁻¹² to 10⁰. So the actual values are: 
    #0.000000000001, 0.00000000001, 0.0000000001, ... all the way up to 1. Each step is 10 times larger than the previous one.
    params = {
    ...
    df_log.to_csv(out_csv_name, mode='w', index=False)
